...
#Updated by Richard Jenkins (User:Firefly) in 2021
import pywikibot
...
class DRNClerkBot:
    ...
    def read_page(self):
        ...
        for section in wikitext.get_sections(levels=[2]):
            title = section.filter_headings()[0].title.strip()
            body = str(section)
           
            params = section.filter_templates(matches="DR case status")[0].params 
            case_status = self.STATUS_NEW
            if len(params) > 0:
                for option, names in self.ALIASES.items():
    ...
    def write_status(self):
        ...
        status_chart = pywikibot.Page(self.site,'User:FireflyBot/DRNStatus')
